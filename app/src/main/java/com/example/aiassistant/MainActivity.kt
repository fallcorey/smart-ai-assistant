...
class MainActivity : AppCompatActivity() {
    ...
    private fun prepareTextForSpeech(text: String): String {
        // Улучшенная подготовка текста для естественной речи
        return text
            // Убираем эмодзи
            .replace(Regex("[🎯🕐📅📆⏰💬🎵📍⚙️🔊☀️🎮📚💰🏥🍳😂🤣😄😊🤭👋🤔🎉🎤🌤️ℹ️✅❌🔍]"), "")
            // Убираем форматирование Markdown
            .replace(Regex("\\*\\*(.*?)\\*\\*"), "$1")
            .replace(Regex("\\*(.*?)\\*"), "$1")
            // Заменяем символы на слова
            .replace("•", " - ")
            .replace("...", ".")
            .replace("  ", " ")
...
Просто напишите или скажите команду!"""
    ...
    private fun addWelcomeMessage() {
        val welcomeMessage = ChatMessage(
            ...
            "Также можете сказать 'быстрее', 'медленнее', 'выше' или 'ниже' для настройки голоса.",
            true
        )
